...
IMGLIB_EXPORT void copyToFloat(cl_context context, cl_command_queue commandQueue, cl_mem fromBuffer, cl_uint fromOffset, cl_mem toBuffer, cl_uint toOffset, cl_uint size, size_t eventCount=0, cl_event *events=nullptr, cl_event *event=nullptr);
...
inline void copyToFloat(::cl::Context &context, ::cl::CommandQueue &commandQueue, ::cl::Buffer &fromBuffer, cl_uint fromOffset, ::cl::Buffer &toBuffer, cl_uint toOffset, cl_uint size, std::vector<::cl::Event> *events, ::cl::Event *event)
{
	copyToFloat(context(), commandQueue(), fromBuffer(), fromOffset, toBuffer(), toOffset, size,
		(events!=nullptr)?(cl_uint)events->size():0,
		(events!=nullptr && events->size()>0)?(cl_event*)&events->front():nullptr,
