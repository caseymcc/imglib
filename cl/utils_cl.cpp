...
void copyToFloat(cl_context context, cl_command_queue commandQueue, cl_mem fromBuffer, cl_uint fromOffset, cl_mem toBuffer, cl_uint toOffset, cl_uint size, size_t eventCount, cl_event *events, cl_event *event)
{
	cl_int status;
	cl_kernel  kernel=getKernel(context, "copyToFloat", "utils.cl", utils_cl.c_str());
	cl_uint index=0;

	status=clSetKernelArg(kernel, index++, sizeof(cl_mem), &fromBuffer);
	assert(status==CL_SUCCESS);
	status=clSetKernelArg(kernel, index++, sizeof(cl_uint), &fromOffset);
	assert(status==CL_SUCCESS);
	status=clSetKernelArg(kernel, index++, sizeof(cl_mem), &toBuffer);
	assert(status==CL_SUCCESS);
	status=clSetKernelArg(kernel, index++, sizeof(cl_uint), &toOffset);
	assert(status==CL_SUCCESS);
	status=clSetKernelArg(kernel, index++, sizeof(cl_uint), &size);
	assert(status==CL_SUCCESS);
