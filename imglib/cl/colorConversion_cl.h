...
void rgbToRGBA(cl_context context, cl_command_queue commandQueue, _ImageType1 &srcImage, _ImageType2 &dstImage, size_t eventCount=0, cl_event *events=nullptr, cl_event *event=nullptr)
{
    ...
    rgbToRGBA(context, commandQueue, nativeSrc, width(srcImage), height(srcImage),
		nativeDst, width(dstImage), height(dstImage), eventCount, events, event);
}

IMGLIB_EXPORT void rgbToGreyScale(cl_context context, cl_command_queue commandQueue, cl_mem &src, size_t srcWidth, size_t srcHeight, Format srcFormat, Depth srcDepth, 
	cl_mem &dst, size_t dstWidth, size_t dstHeight, Format dstFormat, Depth dstDepth, size_t eventCount=0, cl_event *events=nullptr, cl_event *event=nullptr);
...
bool convert(cl_context context, cl_command_queue commandQueue, _ImageType1 &srcImage, _ImageType2 &dstImage, size_t eventCount=0, cl_event *events=nullptr, cl_event *event=nullptr)
{
    ...
    case Format::RGB:
    {
		...
		case Format::GreyScale:
			cl::rgbToGreyScale(context, commandQueue, srcImage, dstImage, eventCount, events, event);
			return true;
        default:
...
template<typename _ImageType1, typename _ImageType2>
void rgbToGreyScale(::cl::Context &context, ::cl::CommandQueue &commandQueue, _ImageType1 &srcImage, _ImageType2 &dstImage, std::vector<::cl::Event> *events=nullptr, ::cl::Event *event=nullptr)
{
	if((location(srcImage)!=Location::OpenCL)||(location(dstImage)!=Location::OpenCL))
	{
		assert(false);
	...
	rgbToGreyScale(context(), commandQueue(), nativeSrc, width(srcImage), height(srcImage), format(srcImage), depth(srcImage),
		nativeDst, width(dstImage), height(dstImage), format(dstImage), depth(dstImage),
		(events!=nullptr)?(cl_uint)events->size():0,
		(events!=nullptr && events->size()>0)?(cl_event*)&events->front():nullptr,
